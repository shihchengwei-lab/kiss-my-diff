...
Benchmark 結果：patch 變小 31%，觸碰檔案少 20%。
...
這裡合併了兩組 benchmark snapshot：8 個 bugfix 任務、4 個模型，baseline 和 `kiss-my-diff` 各跑兩次。合計是 64 次 baseline run 和 64 次 `kiss-my-diff` run。
...
在這個較大的池子裡，較強模型加上 `kiss-my-diff` 後維持 100% 正確率。這是 diff discipline harness，不是正確率保證。
...
從模型層看，coding agent 仍然是在根據脈絡生成下一個 token。`AGENT.md` 是軟約束，不是硬邊界：它改變 next-token 的機率分布，但不會像程式規則一樣機械式強制執行。我測了幾個更重的版本，目的就是找「patch 變小」和「不要漏掉需求」之間的甜蜜點。完整說明見 [`benchmark/context-weighting-experiment.md`](benchmark/context-weighting-experiment.md)。
