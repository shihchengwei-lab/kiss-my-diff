...
KISS 是 Keep It Simple, Stupid。放到 coding agent 的 diff 裡，就是少寫 patch、少碰檔案、不要把小 bugfix 做成小框架。

Benchmark 結果：patch 小 38%，觸碰檔案少 19%。

`kiss-my-diff` 是一份很小的 [`AGENT.md`](AGENT.md)，給 coding agent 用。它把 KISS 變成一份小規則：先讀現有程式、沿用既有模式、做最小且可讀的修改、不要把錯誤或非法狀態藏起來、驗證結果，然後停下來。
